...
when none of the A or B blocks in any subarray contained 2¡ÔA unique values,
which places a hard limit on the number of times this will ACTUALLY need
to binary search and rotate.
according to my analysis the worst case is ¡ÔA rotations performed on ¡ÔA items
once the constant factors are removed, which ends up being O(n)
again, this is NOT a general-purpose solution ¡V it only works well in this case!
...
// (size + 1)/2 ¡V turns into a full-speed standard merge sort since everything fits into the cache
cache_size = (size + 1)/2;
cache = new (std::nothrow) T[cache_size];
if (cache) return;
// sqrt((size + 1)/2) + 1 ¡V this will be the size of the A blocks at the largest level of merges,
...
cache = new (std::nothrow) T[cache_size];
if (cache) return;
// 512 ¡V chosen from careful testing as a good balance between fixed-size memory use and run time
if (cache_size > 512) {
cache_size = 512;
cache = new (std::nothrow) T[cache_size];
if (cache) return;
}
// 0 ¡V if the system simply cannot allocate any extra memory whatsoever, no memory works just fine
cache_size = 0;
}
};
#endif
...
// 1. pull out two internal buffers each containing ¡ÔA unique values
...
// we need to find either a single contiguous space containing 2¡ÔA unique values (which will be split up into two buffers of size ¡ÔA each),
// or we need to find one buffer of < 2¡ÔA unique values, and a second buffer of ¡ÔA unique values,
// OR if we couldn't find that many unique values, we need the largest possible buffer we can get
// in the case where it couldn't find a single buffer of at least ¡ÔA unique values,
...
// we were able to find a single contiguous section containing 2¡ÔA unique values,
// so this section can be used to contain both of the internal buffers we'll need
buffer1 = Range(A.start, A.start + buffer_size);
buffer2 = Range(A.start + buffer_size, A.start + count);
break;
} else if (find == buffer_size + buffer_size) {
// we found a buffer that contains at least ¡ÔA unique values, but did not contain the full 2¡ÔA unique values,
// so we still need to find a second separate buffer of at least ¡ÔA unique values
...
// we found a second buffer in an 'A' subarray containing ¡ÔA unique values, so we're done!
...
// we found a second buffer in a 'B' subarray containing ¡ÔA unique values, so we're done!
...
// this only happens for very small subarrays, like ¡Ô4 = 2, 2 * (2 internal buffers) = 4,
